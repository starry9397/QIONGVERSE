...
function DeferredHainanMap({ language }: { language: Language }) {
  ...
  return <div id={shouldLoad ? undefined : 'hainan-map'} ref={slotRef} className="hainan-map-deferred" data-luoyin-tour-cue="map-reading" data-luoyin-tour-page="map">
    {shouldLoad
...
function App() {
  ...
  useEffect(() => {
    ...
    const onWorldMove = () => showTourCue({ page: 'hall', sectionId: `${activeHall}-world`, cueId: 'hall-orientation', zoneId: activeHall === 'freeTradePort' ? 'free-trade-port' : activeHall })
    window.addEventListener('luoyin-world-move', onWorldMove)
    return () => window.removeEventListener('luoyin-world-move', onWorldMove)
  }, [activeHall])
  ...
  return <div className={activeHall === 'tropical' ? 'site-shell tropical-route-active' : 'site-shell'}>
    ...
    <main id="top">
      ...
      <section className="free-trade-portal" id="free-trade-main-hall" aria-labelledby="free-trade-portal-title" data-luoyin-tour-cue="home-free-trade" data-luoyin-tour-page="home" data-luoyin-tour-zone="free-trade-port">
      ...
      <section className="exhibition" id="exhibition" ref={exhibitionRef} aria-label={inline(language, 'Five immersive halls', '五个沉浸展厅')} data-luoyin-tour-cue="home-exhibition-wheel" data-luoyin-tour-page="home" onMouseEnter={() => setCarouselPointerPaused(true)} onMouseLeave={() => setCarouselPointerPaused(false)} onPointerDown={() => { carouselPointerDownRef.current = true; setCarouselFocusPaused(false) }} onPointerUp={() => { carouselPointerDownRef.current = false }} onFocusCapture={() => { if (!carouselPointerDownRef.current) setCarouselFocusPaused(true) }} onBlurCapture={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setCarouselFocusPaused(false) }}>
      ...
      <section className="experience-feature experience-feature--travel" aria-labelledby="experience-travel-title" data-luoyin-tour-cue="travel-atlas" data-luoyin-tour-page="travel">
      ...
      <section className="experience-feature experience-feature--tide" aria-labelledby="experience-tide-title" data-luoyin-tour-cue="home-beyond-halls" data-luoyin-tour-page="shellsong">
        ...
        <div className="experience-feature-copy">
          ...
          <h2 id="experience-tide-title">{localize(homeExperienceCopy.shellsongTitle, language)}</h2>
          <span>{localize(homeExperienceCopy.shellsongBody, language)}</span>
          <button type="button" onClick={() => openExperience('luoyin-tide')}>{localize(homeExperienceCopy.shellsongAction, language)} <b aria-hidden="true">↗</b></button>
          <small>{localize(homeExperienceCopy.shellsongMeta, language)}</small>
        </div>
      </section>
      <section className="experience-feature experience-feature--market" aria-labelledby="experience-market-title" data-luoyin-tour-cue="market-demo" data-luoyin-tour-page="market">
        ...
        <div className="experience-feature-copy">
          ...
          <h2 id="experience-market-title">{localize(homeExperienceCopy.marketTitle, language)}</h2>
          <span>{localize(homeExperienceCopy.marketBody, language)}</span>
          <button type="button" onClick={() => openExperience('market')}>{localize(homeExperienceCopy.marketAction, language)} <b aria-hidden="true">↗</b></button>
          <small>{localize(homeExperienceCopy.marketMeta, language)}</small>
        </div>
      </section>
    ...
    <LuoyinDesktopPet language={language} visible={petVisible} chatOpen={guideOpen} suspended={guideBlocked} surfaceTone={guideSurfaceTone} onOpenChat={openGuideChat} onCloseChat={closeGuideChat} onClosePet={closeGuidePet} tourCue={activeTourCopy ? { title: activeTourCopy.title, text: activeTourCopy.text, onOpen: () => tourCue && openTourCue(tourCue), onSpeak: () => { void speakTextNow(activeTourCopy.text) }, onDismiss: dismissTourCue } : undefined}>
