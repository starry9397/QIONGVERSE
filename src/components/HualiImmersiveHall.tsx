...
export default function HualiImmersiveHall({ language, onChangeLanguage, onExit, onOpenGuide }: Props) {
  ...
  return <div className="huali-hall" data-avatar-state={avatarState}>
    ...
    <header className="huali-header"><BrandLockup onNavigate={(event) => { event.preventDefault(); onExit() }} /><p>{hallTx(language, view === 'world' ? 'DONGFANG ROSEWOOD / IMMERSIVE HALL' : 'DONGFANG ROSEWOOD / EXHIBIT INDEX')}</p><div><LanguageSelector language={language} onChange={onChangeLanguage} /><button type="button" onClick={onExit}>{tx(language, 'Back to five halls', '返回五个展厅')}</button></div></header>
