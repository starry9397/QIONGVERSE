...
export default function HualiImmersiveHall({ language, onChangeLanguage, onExit, onOpenGuide }: Props) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        splat = new SplatMesh({ url: '/assets/3d/huali/huali world.spz' }) as unknown as { initialized: Promise<unknown>; dispose: () => void }; scene.add(splat as unknown as ThreeTypes.Object3D)
        ...
        avatar = createLuoyinAvatarController({ scene, avatarScene, contactScene, camera, renderer, controls, splat, config: avatarWorldConfigs.huali, onState: setAvatarState }); avatarRef.current = avatar
  ...
  return <div className={`huali-hall${view === 'world' && immersiveUiHidden ? ' immersive-ui-hidden' : ''}`} data-avatar-state={avatarState}>
    ...
    </div><aside className="huali-overlay"><p className="mono-label">{hallTx(language, 'HAINAN PROVINCE / PROJECT-CURATED VIEW')}</p><h1>{tx(language, 'Dongfang Rosewood Hall', '东方花梨厅')}</h1><p>{tx(language, 'A wood-grain archive built from project-supplied images and concept models. It supports material-led looking, not authentication or commerce.', '一间由项目提供图像与概念模型构成的木纹档案馆，支持以材质为线索的观看，不提供鉴定或商业服务。')}</p><span className="huali-status" aria-live="polite">{sceneStatus === 'ready' ? tx(language, '3D world ready', '3D 世界已准备') : sceneStatus === 'loading' ? tx(language, 'Opening 3D world', '正在打开 3D 世界') : tx(language, 'Static hall view ready', '静态展厅视图已准备')}</span></aside><div className="huali-actions"><button type="button" onClick={() => setView('index')}>{tx(language, 'Open exhibit index', '打开展项索引')}</button><button type="button" onClick={() => onOpenGuide(active)}>{tx(language, 'Ask Luoyin', '询问螺音')}</button></div><nav className="huali-exhibit-strip" aria-label={tx(language, 'Visible exhibit navigation', '可见展项导航')}>{hualiExhibits.map((exhibit) => <button type="button" key={exhibit.id} className={active.id === exhibit.id ? 'active' : ''} onClick={() => { triggerPulse(); select(exhibit) }}><span>◌</span><b>{exhibit.title[language]}</b><small>{hualiStatusLabel(exhibit, language)}</small></button>)}</nav><p className="huali-controls">{tx(language, 'Drag to rotate · wheel / pinch to zoom · WASD or arrow keys to move · click the world for a wood-ring resonance', '拖拽旋转 · 滚轮/双指缩放 · WASD 或方向键移动 · 点击大世界触发木纹共鸣')}</p></main> : <ImmersiveExhibitIndex language={language} onChangeLanguage={onChangeLanguage} onExit={onExit} onBack={() => setView('world')} eyebrow={{ en: 'HAINAN PROVINCE / DONGFANG ROSEWOOD', zh: '海南省 / 东方花梨', id: 'PROVINSI HAINAN / KAYU MAWAR DONGFANG', ja: '海南省 / 東方花梨', ko: '하이난성 / 둥팡 화리목', ru: 'ПРОВИНЦИЯ ХАЙНАНЬ / ПАЛИСАНДР ДУНФАН', ar: 'مقاطعة هاينان / خشب الورد دونغفانغ' }} title={{ en: 'Grain & Gesture Archive', zh: '木纹与手势档案', id: 'Arsip Serat dan Gerak', ja: '木目と手のアーカイブ', ko: '결과 손짓 아카이브', ru: 'Архив волокон и жестов', ar: 'أرشيف الحبيبات والإيماءات' }} subtitle={{ en: 'Material, craft and time gathered into a tactile visual study.', zh: '将材质、工艺与时间汇聚为一场可触的视觉研究。', id: 'Material, kerajinan, dan waktu dihimpun menjadi studi visual yang terasa dekat.', ja: '素材と手仕事、時間を触覚的な視覚研究へと集めます。', ko: '재료와 공예, 시간을 촉각적인 시각 연구로 엮습니다.', ru: 'Материал, ремесло и время собраны в осязаемое визуальное исследование.', ar: 'مادة وحرفة وزمن مجتمعة في دراسة بصرية حسية.' }} background="/assets/index-backgrounds/dongfang-rosewood.jpg" items={hualiExhibits.map((exhibit, index) => ({ id: exhibit.id, title: exhibit.title, introduction: exhibit.introduction, status: exhibit.kind === 'model' ? immersiveIndexStatus.concept : immersiveIndexStatus.project, media: exhibit.modelPoster || exhibit.asset, fallback: exhibit.fallback, accent: exhibit.kind === 'model' ? 'gold' : (['rust', 'olive', 'slate'][index % 3] as 'rust' | 'olive' | 'slate'), onOpen: () => select(exhibit) }))} />}
