...
type Props = { language: Language; onChangeLanguage: (language: Language) => void; onExit: () => void; onOpenGuide: (exhibit: VillageExhibit) => void }
...
function VillagePulse({ reduced, pulse }: { reduced: boolean; pulse: { x: number; y: number; key: number } }) {
    ...
    const draw = (now: number) => {
      ...
      pulses.current.forEach((item) => {
        ...
        for (let ring = 0; ring < 3; ring += 1) {
          ...
          context.strokeStyle = `rgba(184, 216, 129, ${(.74 - ring * .14) * (1 - progress)})`; context.lineWidth = Math.max(1, 1.8 * devicePixelRatio); context.stroke()
        }
        if (!reduced) for (let index = 0; index < 12; index += 1) {
          const angle = Math.PI * 2 * index / 12; const distance = (24 + progress * 110) * devicePixelRatio
          context.fillStyle = `rgba(224, 239, 171, ${.62 * (1 - progress)})`; context.fillRect(item.x * devicePixelRatio + Math.cos(angle) * distance, item.y * devicePixelRatio + Math.sin(angle) * distance * .58, 2 * devicePixelRatio, 2 * devicePixelRatio)
        }
      })
  ...
  return <canvas className="village-pulse-layer" ref={canvasRef} aria-hidden="true" />
}

function VillageModelPreview({ exhibit, language, transform, onTransform }: { exhibit: VillageExhibit; language: Language; transform: ModelTransform; onTransform: (value: ModelTransform) => void }) {
  ...
  useEffect(() => {
    const container = host.current; const modelAsset = exhibit.modelAsset
    ...
    void (async () => {
      try {
        ...
        scene.add(new THREE.HemisphereLight(0xdff0bd, 0x0b1910, 2.3)); const light = new THREE.DirectionalLight(0xf0d69a, 2.1); light.position.set(3, 2, 2); scene.add(light)
...
function VillageDetailSheet({ exhibit, language, transform, onTransform, onClose, onAsk }: { exhibit: VillageExhibit; language: Language; transform: ModelTransform; onTransform: (value: ModelTransform) => void; onClose: () => void; onAsk: () => void }) {
  ...
  return <div className="village-sheet-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }} role="presentation"><section className="village-detail-sheet" role="dialog" aria-modal="true" aria-labelledby="village-detail-title">
    <div className="village-sheet-head"><div><p className="mono-label">{villageStatusLabel(exhibit, language)}</p><h2 id="village-detail-title">{exhibit.title[language]}</h2></div><button className="close-button" type="button" onClick={onClose} aria-label={tx(language, 'Close exhibit', '关闭展项')}>×</button></div>
    {exhibit.kind === 'model' ? <VillageModelPreview exhibit={exhibit} language={language} transform={transform} onTransform={onTransform} /> : <img className="village-detail-media" src={exhibit.asset} alt={exhibit.title[language]} onError={(event) => { event.currentTarget.src = exhibit.fallback }} />}
    <p className="village-detail-en">{exhibit.title.en}</p><p>{exhibit.introduction[language]}</p><p className="village-detail-note">{exhibit.note[language]}</p>
    <button className="village-ask-button" type="button" onClick={onAsk}>{tx(language, 'Ask Luoyin about this exhibit', '询问螺音关于此展项')}</button>
  </section></div>
}
...
export default function VillageImmersiveHall({ language, onChangeLanguage, onExit, onOpenGuide }: Props) {
  ...
  return <div className="village-hall" data-avatar-state={avatarState}>
    ...
    </div><aside className="village-overlay"><p className="mono-label">{hallTx(language, 'HAINAN PROVINCE / PROJECT-CURATED VIEW')}</p><h1>{tx(language, 'Beautiful Villages Hall', '美丽乡村厅')}</h1><p>{tx(language, 'A living landscape archive built from project-supplied village scenes and concept models. It supports attentive looking, not site verification or booking.', '一间由项目提供的乡村场景与概念模型构成的景观档案馆，支持细看与想象，不提供现场核验或预订服务。')}</p><span className="village-status" aria-live="polite">{sceneStatus === 'ready' ? tx(language, '3D world ready', '3D 世界已准备好') : sceneStatus === 'loading' ? tx(language, 'Opening 3D world', '正在打开 3D 世界') : tx(language, 'Static hall view ready', '静态展厅视图已准备好')}</span></aside><div className="village-actions"><button type="button" onClick={() => setView('index')}>{tx(language, 'Open exhibit index', '打开展项索引')}</button><button type="button" onClick={() => onOpenGuide(active)}>{tx(language, 'Ask Luoyin', '询问螺音')}</button></div><nav className="village-exhibit-strip" aria-label={tx(language, 'Visible exhibit navigation', '可见展项导航')}>{villageExhibits.map((exhibit) => <button type="button" key={exhibit.id} className={active.id === exhibit.id ? 'active' : ''} onClick={() => { triggerPulse(); select(exhibit) }}><span>◌</span><b>{exhibit.title[language]}</b><small>{villageStatusLabel(exhibit, language)}</small></button>)}</nav><p className="village-controls">{tx(language, 'Drag to rotate · wheel / pinch to zoom · WASD or arrow keys to move · click the world for a landscape pulse', '拖拽旋转 · 滚轮/双指缩放 · WASD 或方向键移动 · 点击大世界触发景观脉冲')}</p></main> : <ImmersiveExhibitIndex language={language} onChangeLanguage={onChangeLanguage} onExit={onExit} onBack={() => setView('world')} eyebrow={{ en: 'HAINAN PROVINCE / BEAUTIFUL VILLAGES', zh: '海南省 / 美丽乡村', id: 'PROVINSI HAINAN / DESA-DESA INDAH', ja: '海南省 / 美しい農村', ko: '하이난성 / 아름다운 농촌', ru: 'ПРОВИНЦИЯ ХАЙНАНЬ / КРАСИВЫЕ ДЕРЕВНИ', ar: 'مقاطعة هاينان / القرى الجميلة' }} title={{ en: 'Living Landscape Archive', zh: '生长中的景观档案', id: 'Arsip Lanskap yang Hidup', ja: '生きる風景のアーカイブ', ko: '살아 있는 풍경 아카이브', ru: 'Архив живого ландшафта', ar: 'أرشيف المشهد الحي' }} subtitle={{ en: 'Village paths, market gestures and layered green spaces held as a moving landscape study.', zh: '将乡村路径、集市动作与层叠绿意保存为一场流动的景观研究。', id: 'Jalur desa, gerak pasar, dan ruang hijau berlapis sebagai studi lanskap yang bergerak.', ja: '村の道、市場の仕草、重なる緑を動く風景研究として記録します。', ko: '마을 길과 시장의 손짓, 겹겹의 초록을 움직이는 풍경 연구로 담습니다.', ru: 'Деревенские тропы, жесты рынка и многослойная зелень в движущемся исследовании ландшафта.', ar: 'مسارات القرى وإيماءات السوق والمساحات الخضراء المتراكبة في دراسة مشهد متحركة.' }} background="/assets/index-backgrounds/beautiful-villages.jpg" items={villageExhibits.map((exhibit, index) => ({ id: exhibit.id, title: exhibit.title, introduction: exhibit.introduction, status: exhibit.kind === 'model' ? immersiveIndexStatus.concept : immersiveIndexStatus.project, media: exhibit.modelPoster || exhibit.asset, fallback: exhibit.fallback, accent: exhibit.kind === 'model' ? 'olive' : (['blue', 'slate', 'rust'][index % 3] as 'blue' | 'slate' | 'rust'), onOpen: () => select(exhibit) }))} />}
    {detail && <VillageDetailSheet exhibit={detail} language={language} transform={transform} onTransform={setTransform} onClose={() => setDetail(null)} onAsk={() => { setDetail(null); onOpenGuide(detail) }} />}
  </div>
}
