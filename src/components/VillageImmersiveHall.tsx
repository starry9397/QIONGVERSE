...
export default function VillageImmersiveHall({ language, onChangeLanguage, onExit, onOpenGuide }: Props) {
  ...
  return <div className="village-hall" data-avatar-state={avatarState}>
    ...
    <header className="village-header"><BrandLockup onNavigate={(event) => { event.preventDefault(); onExit() }} /><p>{hallTx(language, view === 'world' ? 'BEAUTIFUL VILLAGES / IMMERSIVE HALL' : 'BEAUTIFUL VILLAGES / EXHIBIT INDEX')}</p><div><LanguageSelector language={language} onChange={onChangeLanguage} /><button type="button" onClick={onExit}>{tx(language, 'Back to five halls', '返回五个展厅')}</button></div></header>
