...
import { assertLocalizationTree, completeLocalizationTree, inline, localize, type RuntimeLocalized } from '../i18n'
...
const marketDynamicCopy = {
  cartCount: { en: 'Open demo cart with {count} items', zh: '打开含{count}件商品的演示购物车', id: 'Buka keranjang demo dengan {count} item', ja: '{count}点の商品が入ったデモカートを開く', ko: '{count}개 상품이 담긴 데모 장바구니 열기', ru: 'Открыть демо-корзину с товарами: {count}', ar: 'فتح سلة العرض التي تضم {count} عناصر' },
  units: { en: '{count} demo units', zh: '{count}件演示库存', id: '{count} unit demo', ja: 'デモ在庫 {count}点', ko: '데모 재고 {count}개', ru: '{count} демо-единиц', ar: '{count} وحدة عرض' },
  unitsAvailable: { en: '{count} demo units available', zh: '{count}件演示库存可用', id: '{count} unit demo tersedia', ja: 'デモ在庫 {count}点が利用可能', ko: '데모 재고 {count}개 이용 가능', ru: 'Доступно демо-единиц: {count}', ar: '{count} وحدة عرض متاحة' },
  addToCart: { en: 'Add {item} to demo cart', zh: '将{item}加入演示购物车', id: 'Tambahkan {item} ke keranjang demo', ja: '{item}をデモカートに追加', ko: '{item}을(를) 데모 장바구니에 담기', ru: 'Добавить «{item}» в демо-корзину', ar: 'إضافة {item} إلى سلة العرض' },
  shareItem: { en: 'Share {item}', zh: '分享{item}', id: 'Bagikan {item}', ja: '{item}を共有', ko: '{item} 공유', ru: 'Поделиться: {item}', ar: 'مشاركة {item}' },
  addOne: { en: 'Add one {item}', zh: '增加一件{item}', id: 'Tambah satu {item}', ja: '{item}を1点追加', ko: '{item} 1개 추가', ru: 'Добавить одну единицу «{item}»', ar: 'إضافة وحدة واحدة من {item}' },
  removeOne: { en: 'Remove one {item}', zh: '减少一件{item}', id: 'Kurangi satu {item}', ja: '{item}を1点減らす', ko: '{item} 1개 줄이기', ru: 'Убрать одну единицу «{item}»', ar: 'إزالة وحدة واحدة من {item}' },
  added: { en: '{item} added to the demo cart.', zh: '已将{item}加入演示购物车。', id: '{item} ditambahkan ke keranjang demo.', ja: '{item}をデモカートに追加しました。', ko: '{item}을(를) 데모 장바구니에 담았습니다.', ru: '«{item}» добавлен в демо-корзину.', ar: 'تمت إضافة {item} إلى سلة العرض.' },
} satisfies Record<string, RuntimeLocalized>

const marketDynamic = (language: Language, key: keyof typeof marketDynamicCopy, value: string | number) => localize(marketDynamicCopy[key], language).replaceAll('{count}', String(value)).replaceAll('{item}', String(value))
...
export default function TradePage({ language, onChangeLanguage, onExit, onOpenGuide }: Props) {
  ...
  const addToCart = (entry: DemoProduct) => {
    ...
    setToast(marketDynamic(language, 'added', tx(language, entry.title)))
  }
  ...
  const productCard = (entry: DemoProduct) => <article className="demo-product-card" key={entry.id}><div className="demo-product-image"><img src={entry.image} alt={tx(language, entry.imageAlt)} /><span>{t('PROJECT DEMO', '项目演示')}</span></div><div className="demo-product-copy"><p className="market-kicker">{tx(language, entry.category)}</p><h3>{tx(language, entry.title)}</h3><p>{tx(language, entry.summary)}</p><div className="demo-product-meta"><strong>{formatDemoPrice(language, entry.price)}</strong><span>{marketDynamic(language, 'units', entry.stock)}</span></div><div className="demo-product-actions"><button className="market-card-action" type="button" onClick={() => marketPath(`/product/${encodeURIComponent(entry.id)}`)}>{t('View', '查看')}</button><button className="market-icon-action" type="button" onClick={() => addToCart(entry)} aria-label={marketDynamic(language, 'addToCart', tx(language, entry.title))}>+</button><button className="market-icon-action" type="button" onClick={() => void share(tx(language, entry.title))} aria-label={marketDynamic(language, 'shareItem', tx(language, entry.title))}>...</button></div></div></article>

  if (route.view === 'operator') return <div className="market-page"><a className="market-skip" href="#market-main">{t('Skip to main content', '跳至主要内容')}</a>{header}<main id="market-main" className="market-simple"><p className="market-kicker">{t('OPERATOR / REAL COMMERCE RESERVED', '运营 / 真实商业另行保留')}</p><h1>{t('Operator access remains separate from this demo.', '运营权限与本演示商城保持独立。')}</h1><p>{t('This local demo never provides operator privileges or changes the real commerce system.', '本地演示不会提供运营权限，也不会更改真实商业系统。')}</p>{demoNotice}</main></div>
   if (route.view === 'order') return <div className="market-page"><a className="market-skip" href="#market-main">{t('Skip to main content', '跳至主要内容')}</a>{header}<main id="market-main" className="market-simple"><p className="market-kicker">{t('ORDER ACCESS / DEMO BOUNDARY', '订单访问 / 演示边界')}</p><h1>{t('Demo orders exist only in this open session.', '演示订单仅在当前打开的会话中存在。')}</h1><p>{t('There is no email, order code, saved order record or real fulfilment in this demo mode.', '演示模式不发送邮件、不生成订单验证码、不保存订单记录，也不产生真实履约。')}</p>{demoNotice}<button className="market-action" type="button" onClick={() => marketPath()}>{t('Return to market', '返回商城')}</button></main></div>
  if (route.view === 'product' && product) return <div className="market-page"><a className="market-skip" href="#market-main">{t('Skip to main content', '跳至主要内容')}</a>{header}<main id="market-main" className="market-detail"><button className="market-back" type="button" onClick={() => marketPath()}>{t('Back to market', '返回商城')}</button><section className="demo-detail"><div><p className="market-kicker">{tx(language, product.category)} / {t('PROJECT DEMO', '项目演示')}</p><h1>{tx(language, product.title)}</h1><p>{tx(language, product.summary)}</p><p className="demo-detail-price">{formatDemoPrice(language, product.price)} <span>{marketDynamic(language, 'unitsAvailable', product.stock)}</span></p>{demoNotice}<div className="demo-detail-actions"><button className="market-action" type="button" onClick={() => addToCart(product)}>{t('Add to demo cart', '加入演示购物车')}</button><button className="market-text-button" type="button" onClick={() => void share(tx(language, product.title))}>{t('Share item', '分享商品')}</button></div></div><figure><img src={product.image} alt={tx(language, product.imageAlt)} /><figcaption>{t('Project demo visual. It is not a merchant product photo or proof of availability.', '项目演示图。它不是商家商品照片，也不是可售凭证。')}</figcaption></figure></section>{product.story && <section className="demo-story"><figure><img src={product.story.src} alt={tx(language, product.story.alt)} /></figure><div><p className="market-kicker">{t('STORY CONTEXT', '故事语境')}</p><h2>{t('A project study behind the product story.', '商品故事背后的项目研究。')}</h2><p>{t('This 3D study provides cultural and spatial context only. It does not describe a manufactured or shippable item.', '此 3D 研究仅提供文化与空间语境，不描述真实生产或可发货商品。')}</p></div></section>}</main></div>
  if (route.view === 'cart') return <div className="market-page"><a className="market-skip" href="#market-main">{t('Skip to main content', '跳至主要内容')}</a>{header}<main id="market-main" className="market-cart"><p className="market-kicker">{t('CART / SESSION ONLY', '购物车 / 仅限当前会话')}</p><h1>{t('Your demo cart', '你的演示购物车')}</h1>{demoNotice}{cartItems.length ? <><div className="demo-cart-lines">{cartItems.map((line) => <article key={line.productId}><img src={line.product.image} alt={tx(language, line.product.imageAlt)} /><div><h2>{tx(language, line.product.title)}</h2><p>{formatDemoPrice(language, line.product.price)}</p><div className="demo-quantity"><button type="button" onClick={() => updateQuantity(line.productId, line.quantity - 1)} aria-label={marketDynamic(language, 'removeOne', tx(language, line.product.title))}>-</button><span aria-label={t('Quantity', '数量')}>{line.quantity}</span><button type="button" onClick={() => updateQuantity(line.productId, line.quantity + 1)} disabled={line.quantity >= line.product.stock} aria-label={marketDynamic(language, 'addOne', tx(language, line.product.title))}>+</button><button className="market-text-button" type="button" onClick={() => updateQuantity(line.productId, 0)}>{t('Remove', '移除')}</button></div></div><strong>{formatDemoPrice(language, line.product.price * line.quantity)}</strong></article>)}</div><div className="demo-cart-summary"><p>{t('Demo subtotal', '演示小计')} <strong>{formatDemoPrice(language, cartTotal)}</strong></p><button className="market-action" type="button" onClick={() => marketPath('/checkout')}>{t('Continue to demo payment', '继续演示付款')}</button></div></> : <section className="market-empty"><h2>{t('Your demo cart is empty.', '你的演示购物车为空。')}</h2><p>{t('Explore a cultural concept or Luoyin IP item to test the transaction flow.', '浏览文化概念商品或螺音 IP 周边，体验完整交易流程。')}</p><button className="market-action" type="button" onClick={() => marketPath()}>{t('Explore the market', '浏览商城')}</button></section>}</main></div>
  if (route.view === 'checkout') return <div className="market-page"><a className="market-skip" href="#market-main">{t('Skip to main content', '跳至主要内容')}</a>{header}<main id="market-main" className="market-checkout"><p className="market-kicker">{t('CHECKOUT / DEMO ONLY', '结账 / 仅限演示')}</p><h1>{t('Simulate payment', '模拟付款')}</h1>{demoNotice}{cartItems.length ? <form onSubmit={completeCheckout}><section className="demo-checkout-summary"><h2>{t('Order summary', '订单摘要')}</h2>{cartItems.map((line) => <p key={line.productId}><span>{tx(language, line.product.title)} x {line.quantity}</span><strong>{formatDemoPrice(language, line.product.price * line.quantity)}</strong></p>)}<p className="demo-checkout-total"><span>{t('Demo total', '演示总计')}</span><strong>{formatDemoPrice(language, cartTotal)}</strong></p></section><fieldset><legend>{t('Demo delivery details', '演示配送信息')}</legend><label>{t('Name', '姓名')}<input name="name" autoComplete="name" required /></label><label>{t('Email', '邮箱')}<input name="email" type="email" autoComplete="email" required /></label><label>{t('Destination', '目的地')}<input name="destination" autoComplete="country-name" required /></label><p>{t('These fields validate the interface only and are discarded when the page changes.', '这些字段仅用于验证界面，并会在页面变更时丢弃。')}</p></fieldset><fieldset><legend>{t('Demo payment method', '演示支付方式')}</legend><label className="demo-radio"><input type="radio" name="payment" checked={paymentMethod === 'card'} onChange={() => setPaymentMethod('card')} /> {t('Card payment simulation', '银行卡支付模拟')}</label><label className="demo-radio"><input type="radio" name="payment" checked={paymentMethod === 'wallet'} onChange={() => setPaymentMethod('wallet')} /> {t('Wallet payment simulation', '钱包支付模拟')}</label></fieldset><button className="market-action" type="submit">{t('Complete demo payment', '完成演示付款')}</button><button className="market-text-button" type="button" onClick={() => marketPath('/cart')}>{t('Return to cart', '返回购物车')}</button></form> : <section className="market-empty"><h2>{t('There is nothing to simulate yet.', '还没有可模拟付款的商品。')}</h2><button className="market-action" type="button" onClick={() => marketPath()}>{t('Return to market', '返回商城')}</button></section>}</main></div>
  if (route.view === 'receipt') {
    ...
    return <div className="market-page"><a className="market-skip" href="#market-main">{t('Skip to main content', '跳至主要内容')}</a>{header}<main id="market-main" className="market-receipt">{receipt ? <><p className="market-kicker">{receipt.kind === 'order' ? t('DEMO ORDER RECEIPT', '演示订单回执') : t('DEMO PROJECT RECEIPT', '演示项目回执')}</p><h1>{receipt.kind === 'order' ? t('Demo payment complete.', '演示付款已完成。') : t('Project demo created.', '项目演示已创建。')}</h1><p className="demo-reference">{t('Reference', '参考编号')}: <code>{receipt.reference}</code></p>{demoNotice}{receipt.kind === 'order' ? <section><h2>{t('Demo order items', '演示订单商品')}</h2>{receiptProducts.map((entry) => <p key={entry.id}>{tx(language, entry.title)}</p>)}<p>{t('No payment settled, order saved, email sent or shipment created.', '未结算资金、未保存订单、未发送邮件，也未创建发货任务。')}</p></section> : <section><h2>{tx(language, receiptService?.title || { en: 'Selected service', zh: '已选服务' })}</h2><p>{receipt.projectDirection || t('No project direction was provided.', '未提供项目方向。')}</p><p>{t('This is not a quote, agreement, appointment or promise of human follow-up.', '这不是报价、协议、预约或真人跟进承诺。')}</p></section>}<div className="demo-detail-actions"><button className="market-action" type="button" onClick={() => marketPath()}>{t('Continue exploring', '继续浏览')}</button><button className="market-text-button" type="button" onClick={() => void share(receipt.reference)}>{t('Share demo receipt', '分享演示回执')}</button></div></> : <section className="market-empty"><p className="market-kicker">{t('RECEIPT EXPIRED', '回执已失效')}</p><h1>{t('This demo receipt is no longer available.', '此演示回执已失效。')}</h1><p>{t('Demo receipts exist only while this page remains open and are never stored.', '演示回执仅在当前页面打开期间存在，绝不会被存储。')}</p><button className="market-action" type="button" onClick={() => marketPath()}>{t('Return to market', '返回商城')}</button></section>}</main></div>
  }

  return <div className="market-page"><a className="market-skip" href="#market-main">{t('Skip to main content', '跳至主要内容')}</a>{header}<main id="market-main"><section className="market-hero demo-market-hero"><div className="market-hero-copy"><p className="market-kicker">{t('QIONGVERSE DEMO MARKET / HAINAN', 'QIONGVERSE 演示商城 / 海南')}</p><h1>{t('Three ways to turn a cultural story into a commercial journey.', '用三条路径，让文化故事成为商业旅程。')}</h1><p>{t('Explore a virtual-service brief, a Hainan-inspired culture shop and the Luoyin IP collection in one local, no-money transaction demo.', '在同一场本地无资金交易演示中，浏览虚拟服务项目、海南灵感文化商店与螺音 IP 系列。')}</p>{demoNotice}<div><button className="market-action" type="button" onClick={() => openHomeSection('market-services')}>{t('Start with services', '从服务开始')}</button><button className="market-text-button" type="button" onClick={() => openHomeSection('market-culture')}>{t('Explore shop', '浏览商店')}</button></div></div><figure><img src="/assets/zones/tropical/zone-tropical-wide.webp" alt={t('Project-supplied Hainan coastal exhibition context', '项目提供的海南海岸展览语境图')} /><figcaption>{t('Project context image only. The commercial interactions on this page are a local demo.', '仅为项目语境图。本页商业交互均为本地演示。')}</figcaption></figure></section><section id="market-services" className="demo-services" aria-labelledby="services-title"><div className="market-section-heading"><div><p className="market-kicker">{t('PATH 01 / BUSINESS SERVICES', '路径 01 / 商业服务')}</p><h2 id="services-title">{t('Build the next cultural destination.', '构建下一个文化目的地。')}</h2></div><p>{t('For museums, tourism venues, merchants and enterprises. Select a service to simulate a project brief, not a contract.', '面向博物馆、文旅场馆、商家与企业。选择服务以模拟项目需求，不代表合同。')}</p></div><div className="demo-service-grid">{demoServices.map((entry) => <article className="demo-service-card" key={entry.id}><p className="market-kicker">{t('PROJECT DEMO', '项目演示')}</p><h3>{tx(language, entry.title)}</h3><p>{tx(language, entry.summary)}</p><ul>{entry.deliverables.map((item) => <li key={item.en}>{tx(language, item)}</li>)}</ul><button className="market-card-action" type="button" onClick={() => setService(entry)}>{t('Configure demo brief', '配置演示需求')}</button></article>)}</div></section><section id="market-culture" className="demo-shop" aria-labelledby="culture-title"><div className="market-section-heading"><div><p className="market-kicker">{t('PATH 02 / CULTURE SHOP', '路径 02 / 文化商店')}</p><h2 id="culture-title">{t('Objects for carrying a Hainan-inspired story.', '携带海南灵感故事的物件。')}</h2></div><p>{t('All prices, stock and checkout outcomes are project-demo data. Images are supplied placeholders, not merchant listing proof.', '所有价格、库存和结账结果均为项目演示数据。图像为提供的占位素材，而非商家上架凭证。')}</p></div><div className="demo-product-grid">{demoProducts.filter((entry) => entry.collection === 'culture').map(productCard)}</div></section><section id="market-ip" className="demo-ip" aria-labelledby="ip-title"><div className="market-section-heading"><div><p className="market-kicker">{t('PATH 03 / LUOYIN IP', '路径 03 / 螺音 IP')}</p><h2 id="ip-title">{t('Take the guide beyond the exhibition.', '让螺音走出展厅。')}</h2></div><p>{t('Project IP merchandise concepts for the QIONGVERSE story world. Availability is simulated only.', '为琼境故事世界设计的项目 IP 周边概念，仅模拟可售状态。')}</p></div><div className="demo-product-grid demo-ip-grid">{demoProducts.filter((entry) => entry.collection === 'ip').map(productCard)}</div></section><section className="demo-recommendations" aria-labelledby="recommendations-title"><div className="market-section-heading"><div><p className="market-kicker">{t('REFRESH / YOU MAY ALSO LIKE', '刷新 / 你可能喜欢')}</p><h2 id="recommendations-title">{t('Try another route through the collection.', '再走一条探索路线。')}</h2></div><button className="market-icon-button" type="button" onClick={refreshRecommendations} aria-label={t('Refresh demo recommendations', '刷新演示猜你喜欢')}>{t('Refresh', '刷新')}</button></div><div className="demo-product-grid demo-recommendation-grid">{recommendationItems.map(productCard)}</div></section></main><footer className="market-footer"><img src="/assets/brand/qiongverse-wordmark-en.svg" alt="HAINAN QIONGVERSE" /><p>{t('A local competition demo for commercial storytelling. It does not process real commerce.', '用于商业叙事的本地竞赛演示，不处理真实交易。')}</p><button type="button" onClick={() => marketPath('/cart')}>{t('Open demo cart', '打开演示购物车')}</button></footer>{service && <div className="demo-service-modal" role="dialog" aria-modal="true" aria-labelledby="service-dialog-title"><form className="demo-service-sheet" onSubmit={createServiceReceipt}><div className="demo-service-modal-head"><div><p className="market-kicker">{t('LOCAL PROJECT DEMO', '本地项目演示')}</p><h2 id="service-dialog-title">{tx(language, service.title)}</h2></div><button type="button" className="market-icon-action" onClick={() => setService(null)} aria-label={t('Close service configuration', '关闭服务配置')}>x</button></div><p>{t('This form creates a local project-demo receipt only. It sends nothing and does not request human follow-up.', '此表单仅生成本地项目演示回执，不会发送信息，也不会请求真人跟进。')}</p><label>{t('Project direction', '项目方向')}<textarea name="direction" maxLength={360} required placeholder={t('Describe the visitor experience you want to demonstrate.', '描述希望演示的访客体验。')} /></label><button className="market-action" type="submit">{t('Create project demo', '生成项目演示')}</button></form></div>}{toast && <p className="market-toast" role="status" aria-live="polite">{toast}</p>}</div>
}
