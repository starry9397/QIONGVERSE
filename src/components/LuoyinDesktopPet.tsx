...
type Props = {
  ...
  autoGuideCue?: { title: string; text: string; onOpen: () => void; onDismiss: () => void; onSkip: () => void }
  children: ReactNode
}
...
export default function LuoyinDesktopPet({ language, visible, chatOpen, suspended, onOpenChat, onCloseChat, onClosePet, surfaceTone = 'light', tourCue, autoGuideCue, children }: Props) {
  ...
  }, [position, chatOpen, tourCue, autoGuideCue])
  ...
  return <div className={`luoyin-desktop-pet tone-${surfaceTone}${chatOpen ? ' is-chat-open' : ''}${dragging ? ' is-dragging' : ''}${opensDown ? ' chat-opens-down' : ''}${opensRight ? ' chat-opens-right' : ''}`} style={{ left: position.x, top: position.y, '--luoyin-chat-space': `${chatSpace}px` } as CSSProperties}>
    {tourCue && !chatOpen && !autoGuideCue && <aside ref={cueRef} className={`luoyin-tour-cue cue-side-${cuePlacement.side} cue-vertical-${cuePlacement.above ? 'above' : 'below'}`} style={{ '--luoyin-cue-left': `${cuePlacement.left}px`, '--luoyin-cue-top': `${cuePlacement.top}px` } as CSSProperties} role="status" aria-live="polite"><button className="luoyin-tour-cue-close" type="button" onClick={tourCue.onDismiss} aria-label={inline(language, 'Dismiss tour cue', '关闭导览提示')}>×</button><p className="luoyin-tour-cue-title">{tourCue.title}</p><p>{tourCue.text}</p><div className="luoyin-tour-cue-actions"><button type="button" onClick={tourCue.onOpen}>{inline(language, 'Ask Luoyin', '询问螺音')} <span aria-hidden="true">↗</span></button>{tourCue.onSpeak && <button type="button" onClick={tourCue.onSpeak}>{inline(language, 'Play voice', '播放语音')} <span aria-hidden="true">◉</span></button>}<button type="button" onClick={tourCue.onDismiss}>{inline(language, 'Later', '稍后')}</button></div></aside>}
    {autoGuideCue && !chatOpen && <aside ref={cueRef} className={`luoyin-tour-cue luoyin-auto-guide-cue cue-side-${cuePlacement.side} cue-vertical-${cuePlacement.above ? 'above' : 'below'}`} style={{ '--luoyin-cue-left': `${cuePlacement.left}px`, '--luoyin-cue-top': `${cuePlacement.top}px` } as CSSProperties} role="status" aria-live="polite"><button className="luoyin-tour-cue-close" type="button" onClick={autoGuideCue.onDismiss} aria-label={inline(language, 'Dismiss automatic guide', '关闭自动导览')}>×</button><p className="luoyin-tour-cue-title">{inline(language, 'LUOYIN / NEARBY EXHIBIT', '螺音 / 附近展项')}</p><p className="luoyin-auto-guide-title">{autoGuideCue.title}</p><p>{autoGuideCue.text}</p><div className="luoyin-tour-cue-actions"><button type="button" onClick={autoGuideCue.onOpen}>{inline(language, 'Continue learning', '继续了解')} <span aria-hidden="true">↗</span></button><button type="button" onClick={autoGuideCue.onSkip}>{inline(language, 'Skip', '跳过')}</button></div></aside>}
