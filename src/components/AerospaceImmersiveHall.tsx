...
export default function AerospaceImmersiveHall({ language, onChangeLanguage, onExit, onOpenGuide }: Props) {
  ...
  return <div className="aerospace-hall" data-avatar-state={avatarState}>
    ...
    </div><aside className="aerospace-overlay"><p className="mono-label">{hallTx(language, 'HAINAN PROVINCE / PROJECT-CURATED VIEW')}</p><h1>{tx(language, 'Wenchang Aerospace Hall', '文昌航天厅')}</h1><p>{tx(language, 'A launch-horizon reading room built from project-supplied images. Official public sources provide background, not approval or media verification.', '一间由项目提供图像构成的发射地平线阅读室。官方公开来源仅提供背景导览，不构成认可或素材核验。')}</p><span className="aerospace-status" aria-live="polite">{sceneStatus === 'ready' ? tx(language, '3D world ready', '3D 世界已准备') : sceneStatus === 'loading' ? tx(language, 'Opening 3D world', '正在打开 3D 世界') : tx(language, 'Static hall view ready', '静态展厅视图已准备')}</span></aside><div className="aerospace-actions"><button type="button" onClick={() => setView('index')}>{tx(language, 'Open exhibit index', '打开展项索引')}</button><button type="button" onClick={() => onOpenGuide(active)}>{tx(language, 'Ask Luoyin', '询问螺音')}</button></div><nav className="aerospace-exhibit-strip" aria-label={tx(language, 'Visible exhibit navigation', '可见展项导航')}>{aerospaceExhibits.map((exhibit) => <button type="button" key={exhibit.id} className={active.id === exhibit.id ? 'active' : ''} onClick={() => { triggerPulse(); select(exhibit) }}><span>◌</span><b>{exhibit.title[language]}</b><small>{tx(language, 'Project asset', '项目素材')}</small></button>)}</nav><p className="aerospace-controls">{tx(language, 'Drag to rotate · wheel / pinch to zoom · WASD or arrow keys to move · click the world for an orbit pulse', '拖拽旋转 · 滚轮/双指缩放 · WASD 或方向键移动 · 点击大世界触发轨道脉冲')}</p></main> : <ImmersiveExhibitIndex language={language} onChangeLanguage={onChangeLanguage} onExit={onExit} onBack={() => setView('world')} eyebrow={{ en: 'HAINAN PROVINCE / WENCHANG AEROSPACE', zh: '海南省 / 文昌航天', id: 'PROVINSI HAINAN / DIRGANTARA WENCHANG', ja: '海南省 / 文昌宇宙開発', ko: '하이난성 / 원창 우주항공', ru: 'ПРОВИНЦИЯ ХАЙНАНЬ / АЭРОКОСМИЧЕСКИЙ ВЭНЬЧАН', ar: 'مقاطعة هاينان / فضاء ونتشانغ' }} title={{ en: 'Launch Horizon Archive', zh: '发射地平线档案', id: 'Arsip Cakrawala Peluncuran', ja: '打上げ地平線アーカイブ', ko: '발사 지평선 아카이브', ru: 'Архив горизонта запуска', ar: 'أرشيف أفق الإطلاق' }} subtitle={{ en: 'Images, structures and orbital imagination arranged as a project-curated reading room.', zh: '将图像、结构与轨道想象编排成一间项目策展的阅读室。', id: 'Gambar, struktur, dan imajinasi orbit disusun sebagai ruang baca kuratorial proyek.', ja: '画像と構造、軌道の想像力をプロジェクトの読書室として編みます。', ko: '이미지와 구조, 궤도에 대한 상상을 프로젝트 큐레이션 열람실로 구성합니다.', ru: 'Изображения, конструкции и орбитальное воображение собраны в кураторский зал проекта.', ar: 'صور وهياكل وخيال مداري مرتبة كغرفة قراءة ينظمها المشروع.' }} background="/assets/index-backgrounds/wenchang-aerospace.jpg" items={aerospaceExhibits.map((exhibit, index) => ({ id: exhibit.id, title: exhibit.title, introduction: exhibit.introduction, status: immersiveIndexStatus.project, media: exhibit.asset, fallback: exhibit.fallback, accent: ['blue', 'slate', 'gold', 'rust'][index % 4] as 'blue' | 'slate' | 'gold' | 'rust', onOpen: () => select(exhibit) }))} />}
