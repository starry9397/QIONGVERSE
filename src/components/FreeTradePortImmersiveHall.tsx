        ...
        <div className="ftp-sheet-head">
          <div>
            <p className="mono-label">
              PROJECT-SUPPLIED CURATORIAL ASSET / 项目提供的策展素材
            </p>
...
export default function FreeTradePortImmersiveHall({
  ...
  return (
    <div className="ftp-immersive-hall" data-avatar-state={avatarState}>
      {view === "world" && (
      ...
      <header className="ftp-immersive-header">
        <BrandLockup onNavigate={(event) => { event.preventDefault(); onExit(); }} />
        <p>
          {view === "world"
            ? "FREE TRADE PORT / IMMERSIVE HALL"
            : "FREE TRADE PORT / EXHIBIT INDEX"}
        </p>
        <div>
      ...
      {view === "world" ? (
        <main className="ftp-immersive-stage">
          ...
          <aside className="ftp-immersive-overlay">
            <p className="mono-label">
              HAINAN PROVINCE / PUBLIC INFORMATION ORIENTATION
            </p>
            <h1>
        ...
        <main className="ftp-index-page">
          <div className="ftp-index-intro">
            <div>
              <p className="mono-label">HAINAN PROVINCE / FREE TRADE PORT</p>
              <h1>{tx(language, "Exhibit Index", "展项索引")}</h1>
            </div>
            <img src={freeTradePortReferenceImage} alt="" />
            <div>
              <p>
                {tx(
                  language,
                  "Choose a project-supplied visual record. The immersive world remains one step away.",
                  "选择一项项目提供的视觉记录；沉浸大世界始终只需一步返回。",
                )}
              </p>
              <button
                type="button"
                className="ftp-world-return"
                onClick={() => setView("world")}
              >
                {tx(language, "Back to immersive world", "返回沉浸大世界")} ↗
              </button>
            </div>
          </div>
          <div className="ftp-index-list">
            {freeTradePortExhibits.map((exhibit) => (
              <article key={exhibit.id} className="ftp-index-entry">
                <img
                  src={exhibit.asset}
                  alt={exhibit.title[language]}
                  onError={(event) => {
                ...
                <div>
                  <p className="mono-label">
                    {tx(
                      language,
                      "Project-supplied curatorial asset",
                      "项目提供的策展素材",
                    )}
                  </p>
