...
export default function LuoyinTidePage({ language, onChangeLanguage, onExit }: Props) {
  ...
  const sharePoster = async () => {
    ...
    if (navigator.share && (!navigator.canShare || navigator.canShare({ files: [file] }))) {
      try { await navigator.share({ title: 'ShellSong', text: caption, files: [file] }); setShareStatus(ui('shareOpened')); return } catch (error) { if (error instanceof DOMException && error.name === 'AbortError') return }
    }
    setShareStatus(ui('shareUnavailable'))
  }
  const copyCaption = async () => {
    const caption = `${activePosterLines[lineIndex]} — ShellSong / 螺音`
    try { await navigator.clipboard.writeText(caption); setShareStatus(ui('captionCopied')) } catch { setShareStatus(caption) }
  }
